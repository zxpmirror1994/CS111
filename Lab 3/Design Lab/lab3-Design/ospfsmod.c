...
ospfs_dir_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
	...
	while (r == 0 && ok_so_far >= 0 && f_pos >= 2) {
		...
		* EXERCISE: Your code here */ // checked
		int endo = f_pos*OSPFS_DIRENTRY_SIZE - 2*OSPFS_DIRENTRY_SIZE;
		if (endo > dir_oi->oi_size || endo == dir_oi->oi_size) 
		  {
		    r = 1;
		    break;
		  }
		...
		*/
		...
		/* EXERCISE: Your code here */ // checked
		od = ospfs_inode_data(dir_oi, endo);
		if (od->od_ino) 
		  { 
		    uint32_t result;	
		...
		ok_so_far = filldir(dirent, od->od_name, strlen(od->od_name), f_pos, od->od_ino, result);
		  }
	if (ok_so_far < 0)
	  { 
	    r = 0;
...
ospfs_unlink(struct inode *dirino, struct dentry *dentry)
{
  ...
  if (nwrites_to_crash > 0) 
    {                                        
      nwrites_to_crash--;                                             
      eprintk("The number of nwrites_to_crash is %d\n", nwrites_to_crash);  
    }                                                                       
    ...
    {                                            
      eprintk("No deal with nwrites_to_crash\n");                          
      return -EFAULT;                                                 
    }
  //////////////////////////////////
...
allocate_block(void)
{
	/* EXERCISE: Your code here */ // checked
        uint32_t bcount;
        uint32_t fino;
	fino = ospfs_super->os_firstinob;
	bcount = OSPFS_FREEMAP_BLK;
	//check whether there are an free blocks on the disk
	while( bcount < fino) 
	  {
	    void *test = ospfs_block(bcount);
	    int a = 0;
	    while(a < OSPFS_BLKBITSIZE) 
	      {
		if (bitvector_test(test, a)) 
		  {
		    bitvector_clear(test, a); 
		    uint32_t result;
		    result = (bcount - OSPFS_FREEMAP_BLK) * OSPFS_BLKBITSIZE + a;
		    return result;
		  }
		a++;
	      }
...
free_block(uint32_t blockno)
{
	...
	if (ospfs_super->os_firstinob > blockno) 
		return;
	uint32_t mapbcount = blockno / OSPFS_BLKBITSIZE + OSPFS_FREEMAP_BLK;
	uint32_t pos = blockno % OSPFS_BLKBITSIZE;
	if (ospfs_super->os_firstinob > mapbcount) 
	  bitvector_set(ospfs_block(mapbcount), pos);
}
...
indir2_index(uint32_t b)
{
  ...
  if ( b < (OSPFS_NDIRECT + OSPFS_NINDIRECT))
    {
      //b requires to use the doubly indirect block
      return -1;
    }
  else
    {
...
add_block(ospfs_inode_t *oi)
{
	...
	/* EXERCISE: Your code here */
	int32_t indnum2 = indir2_index(n); 
	int32_t indnum = indir_index(n); 
	int32_t num = direct_index(n);

	if (indnum == -1) 
	  {
	    uint32_t bcount = allocate_block();
	    if (!bcount)
	      return -ENOSPC;
	    memset(ospfs_block(bcount), 0, OSPFS_BLKSIZE);
	    oi->oi_direct[num] = bcount;
	  }
	else if (indnum2 == 0) 
	  {
	    if (indnum == 0 && num == 0) 
	    ...
	    uint32_t *indblok2 = ospfs_block(oi->oi_indirect2);
	    
	    if (num == 0) 
	      {
		//An indirect blk, need to free first
		...
		indblok2[indnum] = allocated[1];
	      }
	    uint32_t *indblok = ospfs_block(indblok2[indnum]);
	...
	else if (indnum2 == -1) //no indirect2
	  {
	    if (num == 0) 
	      { 
		//An indirect blk
	    ...
	    uint32_t *indblok = ospfs_block(oi->oi_indirect);
...
ospfs_read(struct file *filp, char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	while (amount < count && retval >= 0) 
	  {
	    uint32_t bcount = ospfs_inode_blockno(oi, *f_pos);
	    uint32_t n;
	    char *data;
		...
		/* EXERCISE: Your code here */ 
	    uint32_t endo = *f_pos % OSPFS_BLKSIZE;
	    n = OSPFS_BLKSIZE - endo;
	    if (n > count - amount)
	      n = count - amount;
	    
	    if (copy_to_user(buffer, data + endo, n))
	      {
		retval = -EFAULT;
		goto done;
	      }
...
ospfs_write(struct file *filp, const char __user *buffer, size_t count, loff_t *f_pos)
{
  ...
  /////////////////////////////////////////////		
	...
	/* EXERCISE: Your code here */ // checked
	uint32_t fileposition = *f_pos + count; // file position after being written
	if (fileposition > oi->oi_size && change_size(oi, fileposition)) {
		retval = -EIO;
		goto done;
	}
	...
	while (amount < count && retval >= 0) {
		...
		if (copy_from_user(temp + endo, buffer, n)) 
		  {
		    retval = -EFAULT;
		    goto done;
		  }
...
create_blank_direntry(ospfs_inode_t *dir_oi)
{
	...
	while(endos < dir_oi->oi_size) 
	  {
	    ospfs_direntry_t* newfile = (ospfs_direntry_t*)ospfs_inode_data(dir_oi, endos);
	    if (newfile->od_ino == 0) 
	      {
		return newfile;
	      }
	    endos += OSPFS_DIRENTRY_SIZE;
	  }
  ...
  /////////////////////////////////////////////		
	...
	//no blank entry in directory
	int test = add_block(dir_oi);

	if (test == 0)    
	  return (ospfs_direntry_t*)ospfs_inode_data(dir_oi, endos);
	else
	  return ERR_PTR(test);
...
ospfs_link(struct dentry *src_dentry, struct inode *dir, struct dentry *dst_dentry) {
 ...
 /////////////////CRASH/////////////////
  ...
  /////////////////////////////////////////////		
	...
	ospfs_inode_t* source = ospfs_inode(src_dentry->d_inode->i_ino);
	ospfs_inode_t* dest = ospfs_inode(dir->i_ino);

	if (find_direntry(dest, dst_dentry->d_name.name, dst_dentry->d_name.len) != NULL)
		return -EEXIST; //check if a file already exists

	if (dst_dentry->d_name.len > OSPFS_MAXNAMELEN)
		return -ENAMETOOLONG; //check if name length too long

	ospfs_direntry_t* ntry = create_blank_direntry(dest);

	if (IS_ERR(ntry))
		return PTR_ERR(ntry); 

	//duplicate name
	memcpy(ntry->od_name, dst_dentry->d_name.name, dst_dentry->d_name.len);
	source->oi_nlink++;
	ntry->od_name[dst_dentry->d_name.len] = 0;
	ntry->od_ino = src_dentry->d_inode->i_ino;
...
ospfs_create(struct inode *dir, struct dentry *dentry, int mode, struct nameidata *nd)
{
 /////////////////CRASH/////////////////
  ...
  /////////////////////////////////////////////		
	...
	/* EXERCISE: Your code here. */
	//if the size of name exceeds the limit
	if (dentry->d_name.len>OSPFS_MAXNAMELEN)
		return -ENAMETOOLONG; 
	...
	//add the inode to that entry
	ospfs_direntry_t* ntry = create_blank_direntry(dir_oi); 
	if (IS_ERR(ntry))
		return PTR_ERR(ntry);
	...
	int inodcount = 2;
	ospfs_inode_t* inod;
	for (; inodcount < ospfs_super->os_ninodes; inodcount++) {
		inod = ospfs_inode(inodcount);
		if (inod->oi_nlink == 0) {
		  //the first inode without any hard links
			break;
		}
	}
	if (inodcount >= ospfs_super->os_ninodes)
		return -ENOSPC; // error checking

	//Initialization
	ntry->od_ino = inodcount;
	memcpy(ntry->od_name, dentry->d_name.name, dentry->d_name.len);
	ntry->od_name[dentry->d_name.len] = 0;
	inod->oi_size = 0;
	inod->oi_ftype = OSPFS_FTYPE_REG;
	inod->oi_nlink = 1;
	inod->oi_mode = mode;
	inod->oi_indirect = 0;
	inod->oi_indirect2 = 0;
...
ospfs_symlink(struct inode *dir, struct dentry *dentry, const char *symname)
{
 /////////////////CRASH/////////////////
  ...
  /////////////////////////////////////////////		
	...
	ospfs_symlink_inode_t *nslink;
	ospfs_direntry_t *nety;
	...
	//if file has already existed
	if (find_direntry(dir_oi, dentry->d_name.name, dentry->d_name.len) != NULL)
		return -EEXIST;


	//get a new direntry 
	nety = create_blank_direntry(dir_oi);
	if (IS_ERR(nety))
		return PTR_ERR(nety);
	...
	//initialization
	nslink = (ospfs_symlink_inode_t*)ospfs_inode(entry_ino);

	nslink->oi_size = strlen(symname);
	nslink->oi_ftype = OSPFS_FTYPE_SYMLINK;
	nslink->oi_nlink++;
	memcpy(nslink->oi_symlink, symname, strlen(symname));
	nslink->oi_symlink[strlen(symname)] = 0;

	nety->od_ino = entry_ino;
	memcpy(nety->od_name, dentry->d_name.name, dentry->d_name.len);
	nety->od_name[dentry->d_name.len] = 0;
	/* Execute this code after your function has successfully created the
	file.  Set entry_ino to the created file's inode number before
	getting here. */
	{
...
ospfs_follow_link(struct dentry *dentry, struct nameidata *nd)
{
	...
	if (strncmp(oi->oi_symlink, "root?", 5) != 0)
	  ...
	  {
		char *endo = strchr(oi->oi_symlink, ':');
		if (endo == NULL)
		  endo = strchr(oi->oi_symlink, '\0');
		else
		  *endo = '\0';	

		reendo = endo - oi->oi_symlink + 1;

		if (current->uid != 0)
		  nd_set_link(nd, oi->oi_symlink + reendo);
		else
