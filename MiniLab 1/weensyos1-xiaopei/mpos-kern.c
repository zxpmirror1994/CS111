...
static pid_t start_thread(process_t *parent, int eip);
...
interrupt(registers_t *reg)
{
	...
	case INT_SYS_WAIT: {
		...
		if (p <= 0 || p >= NPROCS || p == current->p_pid
		  ...
		  {
		    pid_t cur_pid = current->p_pid;
		    proc_array[p].waiting_pid[cur_pid] = 1;
	...
	case INT_SYS_NEWTHREAD: {
	  current->p_registers.reg_eax = start_thread(current, current->p_registers.reg_eax);
	  run(current);
	}
...
do_fork(process_t *parent)
{
  ...
  for(id = 1; id < NPROCS; id++)
    {
      child_proc = &proc_array[id];
      if (P_EMPTY != child_proc -> p_state)
	{
	  continue;
	}
      else
	{
	  child_proc->p_registers = parent->p_registers;
	  copy_stack(child_proc, parent);
	  child_proc->p_registers.reg_eax = 0;
	  child_proc->p_state = P_RUNNABLE;
	  return id;
	}
    }
  return -1;
...
static pid_t
start_thread(process_t *parent, int eip)
{
  pid_t id = 1;
  process_t *child_proc;
  for(id = 1; id < NPROCS; id++)
	...
	{
	  child_proc->p_registers.reg_eip = eip;
	  child_proc->p_registers = parent->p_registers;
	  child_proc->p_registers.reg_eax = 0;
	  child_proc->p_state = P_RUNNABLE;
	  return id;
...
copy_stack(process_t *dest, process_t *src)
{
	...
	// YOUR CODE HERE!
	uint32_t stack_size;
	src_stack_top = PROC1_STACK_ADDR + PROC_STACK_SIZE * (src->p_pid); /* YOUR CODE HERE */;
	src_stack_bottom = src->p_registers.reg_esp;
	stack_size = src_stack_top - src_stack_bottom;
	dest_stack_top = PROC1_STACK_ADDR + PROC_STACK_SIZE * (dest->p_pid); /* YOUR CODE HERE */;
	dest_stack_bottom = dest_stack_top - stack_size; /* YOUR CODE HERE: calculate based on the
				 other variables */;
	// YOUR CODE HERE: memcpy the stack and set dest->p_registers.reg_esp
	memcpy((void*)dest_stack_bottom, (const void*)src_stack_bottom, stack_size);
